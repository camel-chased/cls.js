...
b,c,d,e){null===c&&(c="^\\s?\\@(method|property)[ \\t]+(?:\\{[a-z0-9_\\$\\|]+\\})?[ \\t]?([a-z0-9_\\$]+)?(?:[ \\t]+([a-z]+))?");var h=a.str,r=a.obj,p=RegExp("\\/\\*\\*?\\s*([^\\/]+)","gim");c=new RegExp(c,"gim");for(var f="",A="",f="",l=[],m="",q={};g(l);)if(l=p.exec(h),g(l)&&(m=String(l[1]).replace(/^[\t\*]+/gim,"").replace(/[\*]+/gim,"").replace(/^[ \t\n]{2,50}/gi,"").replace(/\n/gi,"").replace(/(\@)/gi,"\n$1").replace(/^\s+/gim,"").replace(/\s+$/gim,"").trim(),c.lastIndex=0,f=c.exec(m),g(f))){A=
f[1].trim();f=f[2].trim();q[f]={};if("method"===A)m=qa(m,a,d,e),q[m[0]]=m[1];else if("property"===A){var H=d,z=e,t=a.className,u=a.obj,v={},n=RegExp("^\\s?\\@property[ \\t]+\\{([^\\}]+)\\}[ \\t]+([^ \\t\\r\\n]+)[ \\t]*([^\\s\\*\\/]+)?(?:[ \\t]+([^\\n\\/]+))?$","gim"),k=void 0,x=void 0,w=!1;g(H)||(w=!0);k=n.exec(m);if(g(k)){x=k[2];if(!w&&!D(z,methodName,H))throw Error("Cannot override '"+methodName+"' property.");v[x]={};v[x].str=m;v[x].types=k[1].split("|");v[x].value=u[x];v[x].classId="static:"+
t;g(H)&&(v[x].classId=H);v[x].className=t;if(g(k[3])){declarations=[k[3]];g(k[4])&&declarations.push(k[4]);if(w&&-1===declarations.indexOf("static"))throw Error("It should  be static property or classId is undefined.");U(declarations,t,x,!1)&&(v[x].declarations=declarations)}else g(v[x].declarations)||(v[x].declarations=["public"])}else throw Error("I see death people.");G(v[x],t,x,w);g(z)&&(z[x]=v[x]);m=[x,v[x]];q[m[0]]=m[1]}if("undefined"===typeof r[f])throw Error("Class "+A+" [ "+f+" ] is declared but not defined in [ "+
...
function(a,b){var c=q[a].classFacade.getCurrentClassName();if(!this.propertyExists(b))throw Error("There is no property like "+b+" in "+c);var d=this.classProperties[b];if(g(d)){if(d.classId===a){if("function"===l(d.value)){var e=this.classFacades[a],h=q[a];return function(){var c=arguments;a:{var p,f,A="",t=0,m=[];if(g(h.classProperties[b]))if(g(h.classProperties[b].arguments)){p=h.classProperties[b].arguments;if(0===p.length){c=Array.prototype.slice.call(c);break a}for(t in p)f=p[t],Object.prototype.hasOwnProperty.call(p,
t)&&(g(c[t])?m[t]=c[t]:g(f["default"])?m[t]=f["default"]:f.types.indexOf("undefined")?m[t]=void 0:V(h.classInstance.getCurrentClassName(),b,f.name,f.types,"undefined"),A=l(m[t]),"clsClassInstance"===A?(A=h.classFacade.getClassName(),L(m[t],f.types,A,f.name)):-1===f.types.indexOf(A)&&"anytype"!==f.types[0]&&V(h.classInstance.getCurrentClassName(),b,f.name,f.types,A))}else{c=Array.prototype.slice(c);break a}else throw Error("There is no such method like ",b);c=m}c=d.value.apply(e,c);p=q[a];m=p.classProperties;
f=l(c);t=p.classFacade.getClassName();"clsClassInstance"===f&&(f=c.getClassName());if(!g(m[b]))throw Error("There is no method called '"+b+"' in [ "+t+" ] class.");m=m[b].returns;if(!(0<=m.indexOf("anytype"))){if(-1===m.indexOf(f))throw Error("Type mismatch. Method [ "+b+" ] from [ "+t+" ] class, should return '"+m.join(",")+"', not '"+f+"'.");c="clsClassFacade"===f?p.classInstance:c}return c}}return d.value}if(E(d,"public")||g(a)&&E(d,"protected")&&this.mixedWithOrChildOfPropertyOwner(a,b))return q[d.classId].classFacade.getCurrentClassName(),
...
b.charAt(c))})},compress:function(a){return d._compress(a,16,function(a){return b(a)})},_compress:function(a,b,c){if(null==a)return"";var d,f,g={},l={},m="",t="",q="",z=2,w=3,u=2,v=[],n=0,k=0,x;for(x=0;x<a.length;x+=1)if(m=a.charAt(x),Object.prototype.hasOwnProperty.call(g,m)||(g[m]=w++,l[m]=!0),t=q+m,Object.prototype.hasOwnProperty.call(g,t))q=t;else{if(Object.prototype.hasOwnProperty.call(l,q)){if(256>q.charCodeAt(0)){for(d=0;d<u;d++)n<<=1,k==b-1?(k=0,v.push(c(n)),n=0):k++;f=q.charCodeAt(0);for(d=
0;8>d;d++)n=n<<1|f&1,k==b-1?(k=0,v.push(c(n)),n=0):k++,f>>=1}else{f=1;for(d=0;d<u;d++)n=n<<1|f,k==b-1?(k=0,v.push(c(n)),n=0):k++,f=0;f=q.charCodeAt(0);for(d=0;16>d;d++)n=n<<1|f&1,k==b-1?(k=0,v.push(c(n)),n=0):k++,f>>=1}z--;0==z&&(z=Math.pow(2,u),u++);delete l[q]}else for(f=g[q],d=0;d<u;d++)n=n<<1|f&1,k==b-1?(k=0,v.push(c(n)),n=0):k++,f>>=1;z--;0==z&&(z=Math.pow(2,u),u++);g[t]=w++;q=String(m)}if(""!==q){if(Object.prototype.hasOwnProperty.call(l,q)){if(256>q.charCodeAt(0)){for(d=0;d<u;d++)n<<=1,k==
b-1?(k=0,v.push(c(n)),n=0):k++;f=q.charCodeAt(0);for(d=0;8>d;d++)n=n<<1|f&1,k==b-1?(k=0,v.push(c(n)),n=0):k++,f>>=1}else{f=1;for(d=0;d<u;d++)n=n<<1|f,k==b-1?(k=0,v.push(c(n)),n=0):k++,f=0;f=q.charCodeAt(0);for(d=0;16>d;d++)n=n<<1|f&1,k==b-1?(k=0,v.push(c(n)),n=0):k++,f>>=1}z--;0==z&&(z=Math.pow(2,u),u++);delete l[q]}else for(f=g[q],d=0;d<u;d++)n=n<<1|f&1,k==b-1?(k=0,v.push(c(n)),n=0):k++,f>>=1;z--;0==z&&(Math.pow(2,u),u++)}f=2;for(d=0;d<u;d++)n=n<<1|f&1,k==b-1?(k=0,v.push(c(n)),n=0):k++,f>>=1;for(;;)if(n<<=
1,k==b-1){v.push(c(n));break}else k++;return v.join("")},decompress:function(a){return null==a?"":""==a?null:d._decompress(a.length,32768,function(b){return a.charCodeAt(b)})},_decompress:function(a,c,d){var g=[],f=4,l=4,q=3,m="",t=[],w,z,y,u,v,n=d(0),k=c,x=1;for(w=0;3>w;w+=1)g[w]=w;m=0;y=Math.pow(2,2);for(u=1;u!=y;)z=n&k,k>>=1,0==k&&(k=c,n=d(x++)),m|=(0<z?1:0)*u,u<<=1;switch(m){case 0:m=0;y=Math.pow(2,8);for(u=1;u!=y;)z=n&k,k>>=1,0==k&&(k=c,n=d(x++)),m|=(0<z?1:0)*u,u<<=1;v=b(m);break;case 1:m=0;
y=Math.pow(2,16);for(u=1;u!=y;)z=n&k,k>>=1,0==k&&(k=c,n=d(x++)),m|=(0<z?1:0)*u,u<<=1;v=b(m);break;case 2:return""}w=g[3]=v;for(t.push(v);;){if(x>a)return"";m=0;y=Math.pow(2,q);for(u=1;u!=y;)z=n&k,k>>=1,0==k&&(k=c,n=d(x++)),m|=(0<z?1:0)*u,u<<=1;switch(v=m){case 0:m=0;y=Math.pow(2,8);for(u=1;u!=y;)z=n&k,k>>=1,0==k&&(k=c,n=d(x++)),m|=(0<z?1:0)*u,u<<=1;g[l++]=b(m);v=l-1;f--;break;case 1:m=0;y=Math.pow(2,16);for(u=1;u!=y;)z=n&k,k>>=1,0==k&&(k=c,n=d(x++)),m|=(0<z?1:0)*u,u<<=1;g[l++]=b(m);v=l-1;f--;break;
case 2:return t.join("")}0==f&&(f=Math.pow(2,q),q++);if(g[v])m=g[v];else if(v===l)m=w+w.charAt(0);else return null;t.push(m);g[l++]=w+m.charAt(0);f--;w=m;0==f&&(f=Math.pow(2,q),q++)}}};return d}();return t}();"undefined"!==typeof module&&(module.exports=cls);
